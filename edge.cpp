...
{   ifstream fin;
    ofstream fout;
   int c=0,z=0,maxvertex=0,index=1,g=0,k=0;
    ...
    {   
        fin>>triangle[c].a;
        fin>>triangle[c].b;
        if(triangle[c].b>maxvertex)
         maxvertex=triangle[c].b;
        fin>>triangle[c].c;
        if(triangle[c].c>maxvertex)
         maxvertex=triangle[c].c;                          // READING DATA FROM FILE
        fin>>triangle[c].d;
        if(triangle[c].d>maxvertex)
         maxvertex=triangle[c].d;
        g++;
        c++;
        } 
for(int l=1; l<=maxvertex; l++)
{        k=0;
       for(int i=0; i<g-1; i++)
       { 
